...
export default function KnowledgeCardScientific() {
  ...
  if (!card) {
    return (
      <div className="flex min-h-screen items-center justify-center text-stv-muted">
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8] px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-[720px]">
        ...
        {/* ── Title + read time ─────────────────────────────────────── */}
        <h1 className="mb-1 font-bricolage text-2xl font-extrabold leading-snug text-stv-navy">
          {sci.title}
        </h1>
        {sci.readMinutes && (
          ...
          <div className={`mb-7 grid gap-4 ${sci.stats.length === 2 ? 'grid-cols-2' : 'grid-cols-3'}`}>
            {sci.stats.map((stat, i) => (
              <div key={i} className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-center">
                <p className="font-bricolage text-[28px] font-bold leading-none" style={{ color: '#BA7517' }}>
                  {stat.value}
                  {stat.ref && (
        ...
        {sci.references && sci.references.length > 0 && (
          <div className="mt-8 rounded-2xl border border-stv-border bg-white p-5">
            <h3 className="mb-3 font-bricolage text-[15px] font-bold text-stv-navy">Referensi</h3>
        ...
        {nextCard && (
          ...
          >
            <div className="min-w-0">
              ...
              <p className="truncate font-bricolage text-[15px] font-bold text-stv-navy">{nextCard.title}</p>
