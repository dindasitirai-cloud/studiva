...
export default function SubscriptionTier1() {
  ...
  return (
    <div className="mx-auto flex max-w-[760px] flex-col gap-6">
      <div>
        ...
        <p className="text-[14px] text-stv-muted">Kelola paket dan status pembayaran Sekolah Studiva Anda.</p>
      </div>
      ...
      {loading ? (
        ...
        <>
          {message && (
            <p className="rounded-xl bg-stv-sky-tint px-4 py-3 text-[13px] text-stv-body">{message}</p>
          )}

          {!subscription ? (
            <div className="flex flex-col items-center rounded-2xl border-2 border-dashed border-stv-sky-tint py-14 text-center">
              <CreditCard className="h-10 w-10 text-stv-sky/60" strokeWidth={1.5} />
              <p className="mt-3 font-semibold text-stv-navy">Anda belum memiliki subscription</p>
              <Link
                to="/pricing"
                className="mt-4 rounded-full bg-stv-sky-stroke px-5 py-2 text-[14px] font-bold text-white no-underline transition hover:bg-stv-navy"
              >
                Lihat Paket Kami
              </Link>
            </div>
            ...
            <>
              <div className="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-[0_4px_16px_rgba(16,58,107,.06)]">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="rounded-xl bg-stv-sky-tint p-3.5">
              ...
              <div className="flex flex-wrap gap-3">
                <Link
                  to="/pricing"
                  className="flex items-center gap-1.5 rounded-full bg-stv-sky-stroke px-5 py-2.5 text-[14px] font-bold text-white no-underline transition hover:bg-stv-navy"
                >
