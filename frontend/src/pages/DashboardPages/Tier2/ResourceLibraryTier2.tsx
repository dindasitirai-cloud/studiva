...
function ArticleCard({
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl bg-white shadow-[0_4px_16px_rgba(16,58,107,.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_32px_rgba(217,119,6,.14)]">
      {/* Thumbnail — clickable to open article */}
      <button type="button" onClick={onOpen} className="relative flex h-32 items-center justify-center bg-gradient-to-br focus:outline-none" style={{ background: undefined }}>
        <div className={`absolute inset-0 bg-gradient-to-br ${THEME_GRADIENT[article.colorTheme]}`} />
        <Icon className={`relative h-10 w-10 ${THEME_ICON_COLOR[article.colorTheme]}`} strokeWidth={1.5} />
        {isRead && (
      ...
      {/* Body — clickable to open article */}
...
export default function ResourceLibraryTier2() {
  ...
  function handleToggleRead(articleId: string) {
    if (!singleChild) {
      navigate(`${basePath}/resources/${articleId}`);
      return;
    }
    if (isArticleReadByAnyChild(articleId)) {
      unmarkArticleRead(singleChild.id, articleId);
    } else {
      markArticleRead(singleChild.id, articleId);
    }
  }
  ...
  const filtered = useMemo(() => {
    return articles.filter(a => {
      ...
      const matchTab =
        ...
        filterTab === 'sudah-dibaca' ? isArticleReadByAnyChild(a.id) :
        true;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Filter tabs: semua / bookmark / favorit / sudah-dibaca */}
        ...
        {['Semua', ...categories].map(cat => (
          <button
            ...
            className={`rounded-full px-3.5 py-1 text-[12px] font-semibold transition ${
              category === cat
                ? 'bg-stv-navy text-white'
                : 'border border-stv-border bg-white text-stv-body hover:bg-slate-50'
            }`}
          >
            {cat}
          </button>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center rounded-2xl border-2 border-dashed border-amber-200 py-14 text-center">
          ...
          <p className="mt-1 text-[13px] text-stv-muted">
            {filterTab !== 'semua'
              ? 'Gunakan ikon di kartu artikel untuk menambahkan.'
